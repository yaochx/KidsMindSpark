...
def create_outline(payload: dict[str, Any], story_id: str) -> dict[str, Any]:
    safe_concept = make_child_safe_concept(payload["concept"])
...
def create_timeline(story: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, (node_type, title, summary) in enumerate(node_specs):
        next_node_id = f"node_{node_specs[index + 1][0]}" if index < len(node_specs) - 1 else None
        timeline.append(
            {
...
def create_script_pages(story: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for page_number in range(1, 33):
        timeline_node = timeline_by_order[min((page_number - 1) // 4, len(timeline_by_order) - 1)]
            ...
            panels.append(
                {
                    ...
                    "characters": ["主角小队"],
                    "narration": _short_narration(page_number, panel_index),
                    "dialogue": _short_dialogue(page_number, panel_index),
                    "imagePrompt": (
